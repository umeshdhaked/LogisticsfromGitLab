...
import { DataorderService } from 'src/app/services/dataorder.service';
import { DatafromrouteoptimizerService } from 'src/app/services/datafromrouteoptimizer.service';
...
@Component({
  ...
  styleUrls: ['./maindriverdashboard.component.scss'],
})
  ...
  processOrder() {
    ...
    this.orderData.forEach(
      async (order: any) => {
        ...
        this.route = JSON.parse(await localStorage.getItem('route'));
